namespace QualificationWorkForUniversity.Services
{
    public class OrderItemService : BaseDataService<ApplicationDbContext>, IOrderItemService
    {
        ...
        public async Task<int?> AddAsync(string userId, string name, string lastName, BasketModel[] basketItems, string phoneNumber, string email, string country, string region, string city, string address, string postcode)
        {
            return await ExecuteSafeAsync(async () =>
            {
                var orderId = await _orderItemRepository.AddOrderAsync(userId, name, lastName, phoneNumber, email, country, region, city, address, postcode, basketItems.ToList());
